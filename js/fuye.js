function fuye(){
	...
	this.arr=[];
	var i=0;
	this.endtime=new Date('2017/5/3,09:00:00').getTime()
	var t=0;
	var ss=0;
...
//      / t++;
        ...
        var starttime=new Date().getTime();
           ...
           _me.endtime+=(1000*3600*24*t);
            ss=Number(_me.endtime-starttime);
        if( ss<1000){
            t++;
        }


		var hours=Math.floor(ss/1000/3600);
		var min=Math.floor(ss/1000/60%60);
		var sec=Math.floor(ss/1000%60);
//		console.log(sec)
		function totwo(n){
			if(n<10){
				n="0"+n;
	...
	this.loadpic=function(){
		$.post("json/fuye.json",function(data){
			for (var i in data) {
				$(".loadp").append("<div class='longPro'><div class='img fl'><a href='xiangqing.html'><img src='images/fuye/"+data[i]["img"]+"' alt='' /></a><span class='iconfont'><span class='txt'>"+data[i]["txt"]+"</span></span></div><div class='desc fl'><a href='#' class='names2'>"+data[i]["b_detail"]+"</a><div class='title2'>"+data[i]["b_title"]+"</div><div class='timer'><span class='desc1'>距结束还剩：</span><span class='time hours'></span><span class='timetxt'>时</span><span class='time minute' ></span><span class='timetxt'>分</span><span class='time seconds'></span><span class='timetxt'>秒</span></div><div class='purchase'><div class='price'><span class='price_n'><span class='price_y'>¥</span>"+data[i]["b_newprice"]+"</span><span class='price_o'>¥"+data[i]["b_oldprice"]+"</span></div><a href='' class='buy_s'></a></div></div>");
